...
///
/// HIDLINS PATCH — artifact lookup uses `libraryName`, not `packageName`.
/// The crate declares `[lib] name = "hidlins_api"` while the package is
/// `hidlins-api`, so artifacts are `libhidlins_api.*`. Without this the lookup
/// misses every artifact. See CONTRIBUTING.md "Cargokit dependency review"
/// and tools/dev/cargokit-patches/. Every Hidlins edit in the vendored Cargokit
/// tree carries a `Hidlins patch` marker so `grep -rn "Hidlins patch"` after a
/// template re-instantiation lists exactly what must be re-applied.
