//! Test driver: a thin one-shot binary that drives `hidlins-core`
...
fn main() {
    if let Err(err) = run() {
        eprintln!("hidlins-test-driver: {err}");
        std::process::exit(1);
    }
}
...
fn group_rm(args: &[String]) -> Result<(), String> {
    ...
    let behavior = if recurse {
        hidlins_core::GroupDeleteBehavior::Recurse
    } else {
        hidlins_core::GroupDeleteBehavior::Refuse
    };
    vault
...
Vault-level commands (read master password from stdin):
  hidlins-test-driver create <path> --password <stdin>
  hidlins-test-driver dump <path> --password <stdin>
  hidlins-test-driver add-entry <path> --title <t> --username <u> --password <p>
  hidlins-test-driver change-pw <path>

Entry-management commands (read master password from stdin via 'echo PW |'):
  hidlins-test-driver entry add --vault PATH --title T [--group UUID] [--username U] [--password P] [--url U] [--notes N] [--tag T ...]
  hidlins-test-driver entry add-totp --vault PATH --title T --otpauth URI [--group UUID]
  hidlins-test-driver entry add-note --vault PATH --title T --body B [--group UUID]
  hidlins-test-driver entry get --vault PATH --uuid UUID [--show-password]
  hidlins-test-driver entry update --vault PATH --uuid UUID [--title T] [--username U] [--password P] [--url U] [--notes N]
  hidlins-test-driver entry rm --vault PATH --uuid UUID [--purge]
  hidlins-test-driver entry list --vault PATH [--group UUID]
  hidlins-test-driver entry search --vault PATH --query Q [--wildcard]
  hidlins-test-driver entry totp --vault PATH --uuid UUID [--at UNIX_SECONDS]
  hidlins-test-driver entry attach --vault PATH --uuid UUID --name N --file PATH
  hidlins-test-driver entry extract --vault PATH --uuid UUID --name N --out PATH
  hidlins-test-driver entry expire --vault PATH --uuid UUID --when RFC3339

Group commands:
  hidlins-test-driver group create --vault PATH --name N [--parent UUID]
  hidlins-test-driver group rm --vault PATH --uuid UUID [--recurse]
...
fn print_entry_help() {
    println!(
        "\
hidlins-test-driver entry [add|add-totp|add-note|get|update|rm|list|search|totp|attach|extract|expire] ...

Run 'hidlins-test-driver --help' for the full subcommand reference."
    );
}
