...
use hidlins_core::{NoRecoveryConfirmed, Vault, VaultError};
...
fn interrupted_change_master_password_preserves_old_vault() {
    ...
    let mut child = Command::new(env!("CARGO_BIN_EXE_fault_helper"))
        ...
        .arg("old")
        .arg("new")
        .env("HIDLINS_ATOMIC_FAULT_TARGET", &path)
        .env("HIDLINS_ATOMIC_FAULT_SIGNAL_DIR", env.tempdir())
        .env("HIDLINS_ATOMIC_FAULT_PAUSE_PHASE", "fsync_done")
        .spawn()
