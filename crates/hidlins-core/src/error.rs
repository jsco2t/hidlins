//! Single public error type for the `hidlins-core` crate.
//!
...
/// Errors returned by every fallible operation in `hidlins-core`.
#[derive(Debug, thiserror::Error)]
#[non_exhaustive]
pub enum VaultError {
    /// `$HOME` is not set or not resolvable to a path.
    ///
    /// Returned by [`crate::paths::HidlinsPaths::from_env`] when the
