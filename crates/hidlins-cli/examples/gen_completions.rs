...
//! root, using the same code path the runtime `hidlins completions
...
fn main() {
    ...
    for (shell, filename) in [
        (Shell::Bash, "hidlins.bash"),
        (Shell::Zsh, "_hidlins"),
        (Shell::Fish, "hidlins.fish"),
    ] {
        ...
        hidlins_cli::commands::completions::write_completions(shell, &mut writer);
...
/// (`crates/hidlins-cli`); the workspace root is two `..` up.
