//! Hidlins CLI library entry point.
//!
...
// restore the terminal on ^C). Mirrors hidlins-tui's terminal_guard.
#![deny(unsafe_code)]
#![deny(missing_docs)]
...
#[must_use]
pub fn run() -> ExitCode {
    // FR-061 defense-in-depth: if HIDLINS_MASTER_PASSWORD is set in the
