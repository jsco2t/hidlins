//! Phase 4 (T4.2) integration tests for the `hidlins completions
...
use common::{hidlins_bin, run_args};

#[test]
fn completions_bash_nonempty_and_contains_hidlins() {
    let (code, stdout, stderr) = run_args(&["completions", "bash"]);
    assert_eq!(code, 0, "stderr was:\n{stderr}");
    assert!(
        stdout.contains("hidlins"),
...
fn completions_zsh_starts_with_compdef_directive() {
    ...
    assert_eq!(code, 0, "stderr was:\n{stderr}");
    assert!(
        stdout.contains("#compdef hidlins"),
...
fn completions_fish_emits_complete_directive() {
    ...
    assert_eq!(code, 0, "stderr was:\n{stderr}");
    assert!(
        stdout.contains("complete -c hidlins"),
        "fish completion should emit `complete -c hidlins`; got first 200 chars:\n{}",
        truncate(&stdout)
    );
}
...
fn completions_missing_shell_arg_is_user_error() {
    ...
    // `Option<Shell>` (so `hidlins completions --help` works). The
...
fn make_completions_writes_shell_completions_files() {
    ...
    for filename in ["hidlins.bash", "_hidlins", "hidlins.fish"] {
...
fn binary_path_is_set_for_test_harness() {
    // Sanity-check the test environment: `CARGO_BIN_EXE_hidlins`
    // resolves to a path Cargo built for us. If this fails the rest
    // of the file produces confusing errors.
    let path = hidlins_bin();
    assert!(!path.is_empty(), "CARGO_BIN_EXE_hidlins was not set");
}
