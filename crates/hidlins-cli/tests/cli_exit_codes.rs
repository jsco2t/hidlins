...
// `hidlins sync` was a NotImplemented slot through Phase 2; its exit-11
...
/// `hidlins vault list` against an empty registry: exit 0, JSON shape
/// `{"vaults": []}`.
#[test]
...
/// `hidlins vault open --id nonexistent` → exit 1 (`NotRegistered`).
...
/// `hidlins vault create` without `--no-recovery-warning` → exit 1.
#[test]
...
/// `hidlins entry get --uuid <bogus>` against a vault that lacks the
...
/// `hidlins entry list` against a vault with the wrong master password
/// → exit 2 (`vault.locked`).
#[test]
...
/// `hidlins gen password --no-lowercase --no-uppercase --no-digits
