...
//!
//! `hidlins gen` is the only family that does not open a vault, so the
...
fn gen_passphrase_default_human_six_words() {
    // Use a space separator so EFF compound words containing hyphens
    // (e.g. "felt-tip") don't inflate the split count.
    let (code, stdout, _) = run_args(&["gen", "passphrase", "--separator", " "]);
    assert_eq!(code, 0);
    let line = stdout.trim_end();
...
fn gen_password_copy_json_failure_emits_single_error_envelope_on_stdout() {
    ...
    let output = Command::new(env!("CARGO_BIN_EXE_hidlins"))
        .env_remove("DISPLAY")
