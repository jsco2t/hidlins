...
//!
//! Each test invokes a `hidlins vault ... --format json` command
...
//! `crates/hidlins-cli/src/views/vault.rs` ARE the schema; these tests
...
fn vault_create_view_json_schema() {
    ...
    // values (those come from hidlins-core's KdfParams::default and
    // may evolve).
    let kdf = &parsed["kdf"];
