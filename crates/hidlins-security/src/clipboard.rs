...
//! them **only** if they byte-equal the value Hidlins originally wrote
...
pub(crate) fn copy_with_autoclear_inner(
    ...
    let join = std::thread::Builder::new()
        .name("hidlins-clipboard-autoclear".to_owned())
        .spawn(move || {
...
impl AutoClearGuard {
    ...
    /// `hidlins entry get --copy` must keep its process alive until
...
mod tests {
    ...
    fn autoclear_clears_on_timeout_when_contents_match() {
        ...
        // Timer-thread `get_text` returns the same value Hidlins wrote.
