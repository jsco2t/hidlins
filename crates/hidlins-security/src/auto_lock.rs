...
//!
//! [`AutoLockController`] is the heart of `hidlins-security`. The
...
//!
//! [design]: ../../../notebook/projects/hidlins/features/security-behaviors/plans/design.md
//!
//! ## Threading posture
//!
...
pub struct AutoLockController {
    ...
    /// Set when [`Self::tick`]'s drain consumes an affirmative lock event
    /// (`OsLock`/`Manual`). From `Locked`, that transition is invisible in
    /// the returned state even though the *event* is meaningful — e.g. an
    /// OS lock arriving while a caller's unlock KDF is in flight must still
    /// be honored. Consumed via [`Self::take_lock_event_observed`].
...
impl AutoLockController {
    ...
    /// Replace the controller's configuration (e.g., when a different
    /// vault with its own lock settings is opened). Preserves attached
    /// sources and current lock state.
    pub fn set_config(&mut self, config: AutoLockConfig) -> Result<(), SecurityError> {
        config.validate()?;
        self.config = config;
        Ok(())
    }
    ...
    /// True when an affirmative lock event (`OsLock`/`Manual`) was drained
    /// by [`Self::tick`] since the last call. Consuming read — resets to
    /// `false`.
    ///
    /// The `tick` return value cannot carry this: with nothing unlocked the
    /// controller idles in [`LockState::Locked`], so a drained lock event
    /// produces no observable state change — yet a caller with an unlock in
    /// flight must still honor it (hidlins-api marks that unlock's
    /// `lock_pending` so its commit discards instead of presenting).
    pub fn take_lock_event_observed(&mut self) -> bool {
        std::mem::take(&mut self.lock_event_observed)
    }
        ...
        S: OsLockEventSource + 'static,
    {
        ...
        let join = std::thread::Builder::new()
            .name(format!("hidlins-security-{name}"))
            .spawn(move || {
    ...
    /// Signal and join all attached OS-event source threads.
    ///
    /// Idempotent: a second call (including via `Drop`) is a no-op.
...
mod tests {
    ...
    #[test]
    fn drained_lock_events_are_observable_even_from_locked_state() {
        let mut controller =
            AutoLockController::new(AutoLockConfig::default()).expect("valid config");
        let tx = controller.event_tx.clone();

        // Park the controller in `Locked` — the state an unlock's KDF window
        // sees (nothing unlocked yet).
        controller.lock_now(OsLockReason::Manual);
        assert!(
            !controller.take_lock_event_observed(),
            "lock_now is the caller's own act, not a drained event"
        );

        // An OS lock drained while already Locked changes no state, but the
        // event itself must be observable so an in-flight unlock can be
        // marked pending.
        tx.send(SecurityEvent::OsLock {
        ...
        assert!(
            controller.take_lock_event_observed(),
            "drained OsLock must set the observed flag"
        );
        assert!(
            !controller.take_lock_event_observed(),
            "consuming read resets the flag"
        );

        // Activity events must not set it.
        tx.send(SecurityEvent::Activity).expect("send Activity");
        controller.tick(Instant::now());
        assert!(!controller.take_lock_event_observed());
    }
