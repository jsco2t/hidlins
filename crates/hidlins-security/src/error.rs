//! Single public error type for the `hidlins-security` crate.
//!
...
//! `match` arms, and `hidlins-security`'s public API never names a
//! third-party error type.

/// Errors returned by every fallible operation in `hidlins-security`.
///
